...
<div class="p-4 space-y-6">
    ...
    @if($reportData)
        ...
        <!-- Income Analysis -->
        <div class="bg-white dark:bg-gray-900 p-4 rounded-xl border-2 border-gray-200 dark:border-gray-700">
            <h2 class="text-lg font-semibold mb-4 dark:text-white">{{ __('common.income_analysis') }}</h2>
                            ...
                            <div class="bg-gray-200 dark:bg-gray-700 p-3 border-b-2 border-gray-300 dark:border-gray-600">
                                <div class="flex justify-between items-center">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="font-bold text-base dark:text-white">{{ $category['name'] }}</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-base dark:text-white">€{{ number_format($category['total'], 2) }}</div>
                                ...
                                <div class="bg-white dark:bg-gray-900">
                                    <div class="pl-6 pr-2 pb-2 pt-2 space-y-1">
                                        @foreach($category['items'] as $item)
                                            <div class="p-1.5 bg-gray-100 dark:bg-gray-800 rounded text-xs">
                                                <div class="flex justify-between items-center">
                                                    <div class="flex-1">
                                                        <span class="text-gray-400 dark:text-gray-500">└─</span>
                                                        <span class="font-medium dark:text-white">{{ $item['date'] }}</span>
                                                        @if($item['notes'])
                                                            <span class="text-gray-500 dark:text-gray-400 ml-1">- {{ $item['notes'] }}</span>
                                                        @endif
                                                    </div>
                                                    <span class="font-semibold ml-2 dark:text-white">€{{ number_format($item['amount'], 2) }}</span>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
        ...
        <!-- Expenses Analysis -->
        <div class="bg-white dark:bg-gray-900 p-4 rounded-xl border-2 border-gray-200 dark:border-gray-700">
            <h2 class="text-lg font-semibold mb-4 dark:text-white">{{ __('common.expenses_analysis') }}</h2>
                            ...
                            <div class="bg-gray-200 dark:bg-gray-700 p-3 border-b-2 border-gray-300 dark:border-gray-600">
                                <div class="flex justify-between items-center">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="font-bold text-base">{{ $superCategory['name'] }}</span>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-bold text-base dark:text-white">€{{ number_format($superCategory['total'], 2) }}</div>
                                        <div class="text-xs text-gray-600 dark:text-gray-400">
                                            ...
                                            <div class="p-2 bg-gray-50 dark:bg-gray-800">
                                                <div class="flex justify-between items-center">
                                                    <div class="flex items-center gap-2 flex-1">
                                                        ...
                                                        <span class="font-semibold text-sm dark:text-white">{{ $category['name'] }}</span>
                                                        <span class="text-xs text-gray-500 dark:text-gray-400">({{ $category['count'] }})</span>
                                                    </div>
                                                    <div class="text-right">
                                                        <div class="font-semibold text-sm dark:text-white">€{{ number_format($category['total'], 2) }}</div>
