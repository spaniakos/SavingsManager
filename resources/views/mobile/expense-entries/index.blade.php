...
<div class="p-4 space-y-4">
    <h1 class="text-2xl font-bold text-center mb-4">{{ __('common.expense_entries') }}</h1>
    
    <!-- Search and Filters -->
    <div class="bg-white p-4 rounded-xl border-2 border-gray-200 space-y-3">
        <form method="GET" action="{{ route('mobile.expense-entries.index') }}" id="filterForm">
            <!-- Search -->
            <input 
                type="text" 
                name="search" 
            ...
            <select name="super_category_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg mb-3" onchange="document.getElementById('filterForm').submit()">
                <option value="">{{ __('common.all_super_categories') }}</option>
                @foreach($superCategories as $superCategory)
                    <option value="{{ $superCategory->id }}" {{ request('super_category_id') == $superCategory->id ? 'selected' : '' }}>
        ...
        @forelse($entries as $entry)
            <div class="bg-white p-4 rounded-xl border-2 border-gray-200">
                <div class="flex justify-between items-start mb-2">
                    <div class="flex-1">
                        <div class="font-semibold text-lg">€{{ number_format($entry->amount, 2) }}</div>
                        ...
                        @if($entry->is_save_for_later)
                            <span class="inline-block mt-1 px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded">{{ __('common.save_for_later') }}</span>
                        @endif
                    </div>
