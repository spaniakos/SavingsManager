...
<div class="p-4 space-y-4">
    <div class="flex items-center mb-4">
        ...
        <h1 class="text-2xl font-bold">{{ __('common.create') }} {{ __('common.expense_category') }}</h1>
    </div>
    
    <form method="POST" action="{{ route('mobile.expense-categories.store') }}" class="space-y-4">
        @csrf
        ...
        <div class="bg-white p-4 rounded-xl border-2 border-gray-200">
            ...
            <select name="expense_super_category_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                ...
                @foreach($superCategories as $superCategory)
                    <option value="{{ $superCategory->id }}" {{ old('expense_super_category_id') == $superCategory->id ? 'selected' : '' }}>
